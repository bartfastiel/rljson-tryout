...
| `GET /status`                                                    | `{ nodeName, nodeId, identity, publicUrl, domain, role, hubNodeId, hubAddress, peers: [...], nodes: [...], transport, sync, storage, tables: { <table>: rowCount } }`, cross-origin readable; `identity` (slice D7) `{ persistent, startedAt, identityPath }`: `persistent` true when the id was read from the identity file an earlier process left under `DATA_DIR` (it survived a restart), false when this process generated it, `startedAt` the start time the node's announcements carry, which changes with every process while the id stays, `identityPath` the file (`null` without discovery), the whole `null` before discovery started; `role` is `starting`, `standalone`, `hub` or `client`; `peers` from discovery (`nodeId`, `name`, `hostname`, `addresses`, `port`, `role`, `startedAt` as first announced, `firstSeen`, `lastSeen`, `probe`, `excludedFromElection`: whether this node keeps the peer out of its hub election because its reported start time or role disagrees with the peer table, slice D7); `nodes` one entry per `NODE_URLS` URL (`url`, `self`, `name`, `nodeId`, `role`, `connectedClients`, `identity: { persistent, startedAt } \| null` as the node reported it, `reachable`, `lastSeen`, `seenInTopology`); `transport` the hub transport (slice D2): `{ role: "hub", hubAddress, connectedClients, lastError }`, `{ role: "client", hubAddress, connectedToHub, lastError }` or `{ role: "standalone", hubAddress: null, lastError }`; `sync` the change set synchronisation (slice D3): `{ announced, received, skipped, pending, failed, lastError, transfers: [last 10, newest first], catchUp }`, a transfer being `{ direction: "incoming" \| "outgoing", peerNodeId, changeSetHash, changeSetId, tables: { <table>: rowCount }, blobs?: [{ blobId, bytes }], durationMs, at, status: "completed" \| "pending" \| "failed", error? }` (`blobs` the blobs the pull fetched with the rows, slice D5, absent when none) and `catchUp` the last catch-up with a peer (slice D4): `{ lastStartedAt, lastCompletedAt, missingAtStart, pulled, durationMs }`, the times `null` before the first one and `lastCompletedAt` and `durationMs` `null` while it runs |
| `GET /api/stats`                                                 | `{ nodeName, seedSize, uptimeSeconds, startedAt, rssBytes, tables: { <table>: rowCount } }`, cross-origin readable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `GET /api/species`                                               | List of current species versions, `[{ id, hash, name, latinName, description, imageUrl }]`, `imageUrl` the relative path of the image endpoint below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `GET /api/species/:hash/image`                                   | The image bytes of that species version with the `Content-Type` the version names (`image/png` for a seed badge, `image/png` or `image/jpeg` for an upload), `Cache-Control: public, max-age=31536000, immutable` (the hash names the version, the version names the image by content); served from the node's blob store, else pulled through the network's blob cascade and cached, else rendered from the species id for a seed species; `404` for a hash no species version has and `404` with the blob id and the reason for a version whose image no node holds (slices B12 and D5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `POST /api/species/:id/image`                                    | Body the raw bytes of a PNG or JPEG with `Content-Type: image/png` or `image/jpeg`, at most one mebibyte; the bytes decide (PNG signature, JPEG start-of-image marker); writes a new version of the species naming the blob in `imageBlobId` and `imageMimeType`, chained onto the current one, with one change set, and answers `200` with the version as `GET /api/species` lists it (the current version unchanged when it names these bytes already); `415` for another media type or bytes that are not the declared image, `413` for a bigger body, `400` for an empty one, `404` for an unknown id (slice D5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `GET /api/traits`                                                | List of current trait versions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `GET /api/animals?species=<id>&breeder=<id>&trait=<id>&q=<text>` | One page of the current animal versions with species and breeder joined (`speciesId`, `speciesName`, `speciesImageUrl`, `breederId`, `breederFarmName`), `{ items, total, limit, offset }`; `q` matches the name or species name case-insensitively, `limit` (default 50, at most 200) and `offset` (default 0) select the page, a value out of range answers `400`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `GET /api/animals/:id?version=<hash>`                            | Current version with species (`speciesImageUrl` included), breeder and traits joined; `version` selects an older one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `GET /api/sync/transfers?peer=<nodeId>&limit=<n>`                | The last change set transfers of this node, newest first, each as `/status.sync.transfers[n]` lists it; the `SyncAgent` remembers 50; `peer` keeps the transfers with that node (the ones naming it plus a hub's announcements to every client, `peerNodeId` null), `limit` (default 10, at most 50) caps the list and a value out of range answers `400`; cross-origin readable (slice D3b)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `GET /api/change-sets/:hash`                                     | A change set this node holds with the content of its rows, `{ hash, id, items: [{ table, ref, row, previousRow }] }`: `row` the stored row (`null` for a row the store lacks), `previousRow` the row of the version the change set's InsertHistory row names in `previous`, resolved through the local store (`null` for a first version, a predecessor this node does not hold, and every InsertHistory row); `404` for a change set this node does not hold (slice D3b)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `GET /api/events`                                                | Server-sent events (`text/event-stream`, `Cache-Control: no-cache`, `X-Accel-Buffering: no`, cross-origin readable): `insert` when this node writes a change set itself, `data` `{ changeSetHash, changeSetId, tables: { <table>: rowCount }, entityIds }` (the rows the change set names per table, history tables included, and the ids of the entities written); `sync` for every transfer of the `SyncAgent`, `data` one transfer as `/status.sync.transfers[n]` lists it, once with `status: "pending"`, `durationMs: 0` and no `error` when a pull starts and once with its outcome; `topology` whenever the network part of `/status` changes, `data` `{ nodeId, role, hubNodeId, hubAddress, peers, nodes, transport }` (the `lastSeen` and probe timings that advance on their own do not count as a change); `conflict` reserved for slice D11, not emitted yet. Every event carries an `id` that counts up per process; the stream opens with `retry: 3000` and a comment, a comment heartbeat follows every 15 s, and the server ends every stream when it shuts down                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Table          | Columns                                                                                                               |
| -------------- | --------------------------------------------------------------------------------------------------------------------- |
| `species`      | `name`, `latinName`, `description`, `imageBlobId` (the `Bs` id of the PNG, `hshBuffer` of its bytes), `imageMimeType` |
...
- [x] **B12 Species images as blobs.** Depends on: B10. Procedural PNG per
...
/api/species/:hash/image`, images in list and detail. Tests: PNG
      ...
      the phone. Deviation: the encoder compresses with zlib's `Z_RLE`
      strategy over the `Sub` filter and the rasterizer avoids the `Math`
      functions whose rounding engines may differ, because the blob id is
      part of the species row and every node must render the same bytes
      (`docs/findings/blobs.md`); the CRC-32 comes from `node:zlib` too
      rather than a local table. The image is served from the store's own
      `Bs`, which `main.ts` shares with the hub transport, with an
      immutable cache header, and a node whose in-memory `Bs` lost a blob
      (a `sqlite` node after a restart, until C2) renders it again from
      the species id. The species list gained `imageUrl`, the animal list
      and detail `speciesImageUrl`; the seed hashes changed, so persistent
      nodes need recreated volumes to reseed until a migration story exists.
- [x] **B13 Live updates.** Depends on: B8. `GET /api/events` with
      ...
      Deviation: the stream carries three events from the start, `insert`
      from the store's change set event (with the ids of the entities
      written), `sync` from a listener hook on the `SyncAgent` (every
      transfer, plus the moment a pull starts) and `topology` from a watch
      over the network part of `/status` (section 2.5); `conflict` is
      documented for D11 and not emitted. The app refreshes on `insert` and
      on a completed incoming `sync` that touches the tables a view shows,
      debounced, in place (search text, filters, loaded pages, scroll
      position and focus stay; an older version of an animal stays as it
      is), and the network view follows `topology` and `sync` through the
      status feed, whose poll drops to every thirty seconds as the fallback
      and the schedule of the browser probes. The header indicator is a
      polite live region rather than a `status` role, which every view
      keeps for its own messages. Findings, the wire format and what
      Traefik does with an idle stream are in
      `docs/findings/live-updates.md`.
...
- [x] **D3b Transfer indicators.** Depends on: D3, B13. Per partner node in
      ...
      animate and the popup lists it. Deviation: the partner badge of the
      header became a button (the node's link moved into the popup's
      title), the popup reads its first ten transfers from the new
      `GET /api/sync/transfers?peer=` (the `SyncAgent` remembers fifty)
      and follows the `sync` events from then on, a row expands to the
      change set's payload from the new `GET /api/change-sets/:hash`
      (every row's fields; an edited entity next to the version it
      supersedes with the changed fields marked, long texts cut to the
      changed region; history rows in one line), the network view's
      partner cards carry the same icons and a button for the same
      popup, and a hub's announcement to every client (`peerNodeId`
      null) shows on every partner's downstream icon. The Playwright
      tests drive the stream with one fulfilled `text/event-stream`
      response per batch of events and a short `retry`, so that the
      test decides when the page hears each event
      (`docs/findings/live-updates.md`).
- [x] **D4 Bootstrap and heartbeat.** Depends on: D3. Late joiners receive
      ...
      after a restart in production. Deviation: the latest change set is not
      enough for a node that restarts or joins late (it misses everything
      the other clients wrote while it was away), so every attach of a
      client to a hub, in both directions and after every reconnection,
      runs a catch-up: the two nodes list each other's change sets through
      the peer store alone (a table dump, never the read cascade), pull
      what they lack in the order the peer learned about them and announce
      what the peer lacks; the D3 replay of everything a process wrote and
      the hub's repeats on join are gone, and a peer that holds the same
      change sets costs one table read. A ready handshake on the socket
      (`petshop:ready`, the one event this project adds next to the
      library's) keeps either side from listing the other before it is
      listening, because a lost peer request waits thirty seconds.
      `bootstrapHeartbeatMs` was measured and left off: the library's
      bootstrap, its heartbeat and `seedLatestRef` carry exactly one
      reference, the latest, which the catch-up covers, and the heartbeat
      would cost every client a lookup per interval for nothing; the gap
      fill needs `causalOrdering` and replays the hub's ref log only after
      the next live announcement (slices D9 and D10). `/status.sync` gained
      `catchUp`. The Gherkin feature `bootstrap.feature` runs "node3
      restarts and catches up" in-process and against Compose and "a hub
      restarts and learns what the clients hold" and "a node joining with
      a populated store announces what the hub lacks" in-process, where
      roles are given by hand. The non-blocking findings of the D3 review
      landed here too: `tables` on pending transfers, one retry log line
      per round, `maxChangeSetItems` (10 000) and an immediate failure for
      items naming unknown tables, a guard in `recordReceivedChangeSet`,
      and the test holding the hash-rejection message against the real
      cascade, which showed that the cascade words it as the validator
      does (`is wrong. Should be`), not as D3 matched. A change set is
      applied at once since this slice: the agent pulls its rows from
      the peer stores alone (no cascade write-back), data rows before
      history rows, and writes them in one `Io.write`, so a node's own
      store shows a change set whole or not at all, after CI caught a
      client answering an invoice without its items mid-pull. Measured on
      Compose: a restarted client holds what the others wrote 31 ms after
      it reconnected, a node with an empty store pulls 547 change sets
      from a `medium` hub in about 400 ms while `/health` stays under 25 ms
      (`docs/findings/change-set-sync.md`).
- [x] **D5 Blob synchronisation.** Depends on: D3. Images pulled through
      `BsPeer` via the hub, cached locally. Done when a species image uploaded
      to node1 renders on node3. Deviation: since every node renders the
      seed images itself, the slice needed a blob that exists on one node
      only, so it added the upload `POST /api/species/:id/image` (raw PNG
      or JPEG bytes with their media type, checked by the first bytes, one
      mebibyte at most, a new species version with a change set like an
      animal edit; the same bytes twice make no second version) and an
      "Upload image" control on every species card that takes a phone's
      photo. The pull goes through the `BsMulti` of the role (`client.bs`,
      `server.bs`), which the transport hands the store like the read
      cascade; the multi caches a hit locally at every hop on its own, and
      the store checks the bytes against the id before it counts them.
      The `SyncAgent` pulls the blobs a received row names after the rows
      and lists them on the transfer as `blobs: [{ blobId, bytes }]`,
      which the transfer popup shows as "blob 10.6 kB"; a blob nobody
      serves leaves the change set applied and the image endpoint asks
      the network on demand, answering `404` with the reason when no node
      has it, and renders an image from the id only for a seed species.
      Animals keep the species version they reference, so their cards
      show the old badge until they are edited. `BsMulti` fails a miss
      through a peer with a `TypeError` (the not-found error serialises
      as `{}`), so the store asks `blobExists` first; the details, the
      timings (a 10.6 kB blob with its version in 9 to 12 ms on Compose)
      and the upstream candidates are in `docs/findings/blobs.md`.
...
- [x] **D7 Identity persistence.** Depends on: D6. Node ids survive restarts
      on nodes with a volume, the memory node gets a fresh id; findings on how
      the hub treats the returning and the new identity. Deviation: built
      before D6, since the quick restart of a hub with a persistent id is
      the identity case the D1 finding left open; the Compose setup runs
      node1 and node2 over SQLite on named volumes like production for
      it. `/status` reports `identity` (whether the id was restored from
      the identity file or generated at this start, the start time the
      announcements carry, the file), the network view shows "persistent
      id" or "fresh id" per node, and `peers[].excludedFromElection`
      shows the mitigation: `@rljson/network` never refreshes a known
      peer's start time, so the survivors of a hub that restarted within
      the broadcast timeout kept it as hub while it followed another node,
      and a quickly restarted client's stale entry poisoned the next
      election as well (rljson issue 07 stays open upstream);
      `TopologyRepair` compares the peer table with what every node
      reports about itself through the directory's polls and keeps a peer
      whose start time moved, or a hub that denies its role for longer
      than the peer timeout, out of this node's election with
      `excludeFromElection`, the one means the library offers that changes
      nothing this node announces (a manager restart would give this node
      a new start time and the same stale entry on every other node), at
      most once per minute per peer and cause, logged with the cause. The
      returning node needs nothing: its table is fresh. Measured on
      Compose: a hub restarted in under a second is agreed upon again by
      all three nodes within 13 s (before: never), a SQLite client keeps
      its id and is connected again 3 s after the restart, a replaced
      memory node's old id leaves the others within 19 s and everyone is
      connected again within 22 s, unchanged by the repair.
      `features/identity.feature` runs the three cases against Compose and
      the survivor's side in-process; timings before and after in
      `docs/findings/network-discovery.md`.
