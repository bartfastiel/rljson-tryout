...
import { animalCards, speciesFilterChips } from './support.ts';
...
test('shows an error with a retry button when the node answers 500', async ({
  ...
  await expect(animalCards(page)).toHaveCount(0);
