...
describe.each<TraitRelationMode>(['multi-reference', 'junction'])(
  ...
  (traitRelationMode) => {
    ...
    beforeEach(async () => {
      store = new PetShopStore({ traitRelationMode });
    ...
    describe('getAnimalHistory', () => {
      it('returns undefined for an unknown id', async () => {
        expect(await store.getAnimalHistory('no-such-animal')).toBeUndefined();
      });
    ...
    describe('updateAnimal', () => {
      ...
      it('re-creates the junction rows for the new animal hash as new versions of each pairing', async () => {
        ...
        expect(forNewVersion.map((pairing) => pairing.id).sort()).toStrictEqual(
          updated.traits.map((trait) => `sir-quackington--${trait.id}`).sort(),
        );
      ...
      it.each<[string, Record<string, unknown>, string]>([
        ...
        async (_description, changes, message) => {
          ...
          expect(await store.getAnimalHistory('donald-the-third')).toHaveLength(
            1,
          );
        },
      );
...
describe('PetShopStore lists resolve through the current-version rule', () => {
  ...
  const insertVersion = async (
    ...
    const route = Route.fromFlat(
      previousTimeId === undefined ? tableKey : `${tableKey}@${previousTimeId}`,
    );
  ...
  it('lists the new version of a trait once and keeps filtering by the trait id through every version', async () => {
    ...
    expect(await store.listAnimals({ traitId: 'fiercely-loyal' })).toHaveLength(
      carriersBefore,
    );
  });
