 ...
 * three seeded species by its `_hash`. Every animal's `backgroundStory`
 * cross-references at least one Duckburg regular (Scrooge McDuck, Donald
 * Duck, Gyro Gearloose, the Beagle Boys, Magica De Spell or Grandma Duck)
 * and at least one other seeded animal; Sir Quackington's story is the
 * hand-written multi-paragraph outlier of several thousand characters that
 * exercises slice B4's long-content path. The rows are hashed here so that
